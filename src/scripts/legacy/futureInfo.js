...
async function main (){
  ...
  console.log(client)
  console.log(await (setupStakingExtension((await StargateClient.connect(config.rpc)).queryClient)).staking.params())

  // Example queries
  //const balances = await client.bank.balances(myAddress);
  console.log('Staking redelegations: ', await client.staking.validators("BOND_STATUS_UNSPECIFIED"));
